...
document.addEventListener('DOMContentLoaded', () => {
  const calcBtn = document.getElementById('calculate-eligibility');
  ...
  if (calcBtn) {
    calcBtn.addEventListener('click', async (e) => {
      ...
      if (window.location.pathname.includes('degree')) {
        programType = 'degree';
      } else if (window.location.pathname.includes('diploma')) {
        programType = 'diploma';
      } else if (window.location.pathname.includes('certificate')) {
        programType = 'certificate';
      } else if (window.location.pathname.includes('kmtc')) {
        programType = 'kmtc';
      }
      
      await window.calculator.calculateAll(user.id, programType);
  ...
  if (downloadBtn) {
    downloadBtn.addEventListener('click', () => {
      window.calculator.downloadResults();
    });
  }
  
  if (shareBtn) {
    shareBtn.addEventListener('click', () => {
      window.calculator.shareWhatsApp();
    });
  }
  
  if (printBtn) {
    printBtn.addEventListener('click', () => {
      window.calculator.printResults();
    });
  }
  ...
  if (searchInput) {
    searchInput.addEventListener('input', (e) => {
      if (window.calculator) {
        window.calculator.filterResults(e.target.value);
      }
    });
  }
